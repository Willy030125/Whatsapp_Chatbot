  ...
  try {
    ...
    */
    ...
    switch (command) {
      ...
      case 'contact':
      case 'ownerbot':
        return await client.reply(from, '*Bot owner:*\n\n wa.me/6282112378872 \n\n willy030125@gmail.com', id);
        break;
      ...
      case 'add':
        if (!isGroup) return await client.reply(from, '_⛔ Group only!_', id);
        if (!isAdmin) return await client.reply(from, '_⛔ Admin only!_', id);
        if (arguments.length !== 1) client.reply(from, `_⚠️ Contoh Penggunaan perintah : ${botPrefix}add 62812....._`, id);
        if (!isBotAdmin) return await client.reply(from, '_⚠️ Bot is not admin!_', id);
        ...
        if (isNumberValid.status === 200)
          await client
            ...
            .then(async () => await client.reply(from, '_🎉 Welcome, we hope you bring a Pizza here!_', id))
            .catch(async (error) => await client.reply(from, '_🥺 Failed to add, please contact the owner!_', id));
        break;
      ...
      case 'kick':
        ...
        if (isKicked) return await client.reply(from, '_🎉 Selamat jalan, semoga diampuni dosanya :v_', id);
        break;
      ...
      case 'promote':
        ...
        if (isPromoted) return await client.reply(from, '_🎉 Promote is done, congrats!_', id);
        break;
      ...
      case 'demote':
        ...
        if (isDemoted) return await client.reply(from, '_🎉 Demote is done!_\n\n_Press F to pay respect_', id);
        break;
      ...
      case 'revoke':
        ...
        await client
          ...
          .then(async (res) => await client.reply(from, '_🎉 Group invite link has been reset_', id))
...
`
      ...
      case 'kickme':
        if (!isGroup) return await client.reply(from, '_⛔ Group only!_', id);
        if (isOwner) return await client.reply(from, '_⛔ Masa iya owner group mau dikick!_', id);
        await client.reply(from, '_😏 Enak aja mau keluar gitu aja!_', id);
      ...
      case 'gifstiker':
        ...
        if (!quotedMsg.mimetype.includes('mp4')) return await client.reply(from, '_⚠️ Pastikan yang anda kirim adalah file video ber-ekstensi mp4_', id);
        ...
        await client.sendMp4AsSticker(from, vidb64, { fps: 10, startTime: `00:00:00.0`, endTime: `00:00:5.0`, loop: 0, crop:false});
        break;
      ...
      case 'musik':
      case 'music':
        await client.reply(from, "Sabar cok, jangan buru-buru", id);
      ...
      case 'ytmp4':
        await client.reply(from, "Sabar cok, jangan buru-buru, tunggu yak...", id);
      ...
      case 'anime':
        ...
        if (getAnime.errors){
          await client.reply(from, getAnime.error, id)
        } else {
      ...
      case 'wikien':
        ...
        const getWiki = await _function.wiki.wikien(arguments.join(' '));
      ...
      case 'wikiid':
        if (arguments.length < 1) return await client.reply(from, `_⚠️ Contoh Penggunaan Perintah : ${botPrefix}wikiid <keywords>_`, id);
        const getWikiid = await _function.wiki.wikiid(arguments.join(' '));
        if (getWikiid.status !== 200) return await client.reply(from, `_⚠️ *${arguments.join(' ')}* pada Wikipedia tidak ditemukan_`, id);
        await client.reply(from, getWikiid.result, id);
        break;
...
Dito @6285155277438⁩`
	...
	if (!q.includes('|')) return await client.reply(from, ind.wrongFormat(), id);
      ...
      case 'nhder':
        if (disablecommand) return await client.reply(from, "Fitur dimatikan sementara, tobat cok")
...
Usage: *${botPrefix}reminder* 10s | pesan_pengingat
      ...
      case 'animewall':
        if (disablecommand) return await client.reply(from, "_Eits, tydack bisa mas..._", id)
          ...
          _function.weeaboo.wait(imageLink)
            .then(async (result) => {
              if (result.result && result.result.length <= 2) {
                ...
                if (similarity < 0.85) {
                  client.reply(from, 'Low similiarity. \n\nMungkin anda bisa mencoba crop terlebih dahulu atau coba lagi dengan gambar lain', id)
                } else {
      ...
      case 'waifu':
        ...
        if (arguments[0] === 'nsfw' && disablecommand) return await client.reply(from, "Jangan sange dong!", id)
        ...
        if (arguments[0] === 'help' && arguments.length == 1 && disablecommand) return await client.reply(from, ind.waifuex(), id)

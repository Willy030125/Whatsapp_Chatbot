...
const youtubeMusic = async (title) => {
  return new Promise(async (resolve, reject) => {
      ...
      await fetchJson(`http://priv.luii-index.tech:2233/convert.php?youtubelink=${videourl}`)
          .then((res) => resolve({
            result : res,
            duration : dur,
            thumbnail : thumb}))
...
const youtubeVideo = async (title) => {
  return new Promise(async (resolve, reject) => {
    ...
    await fetchJson(`http://priv.luii-index.tech:2233/convert.php?format=mp4&youtubelink=${videourl}`)
        .then((res) => resolve({
          result : res,
          duration : dur,
          thumbnail : thumb}))
